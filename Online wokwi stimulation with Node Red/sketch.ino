...
void connectToBroker(){
  while(!mqttClient.connected()){
    ...
    if(mqttClient.connect("ESP32-465456454")){
      ...
      display.clearDisplay();
      print_line("After Servo",2,0,0);
      mqttClient.subscribe("Sampling");
      mqttClient.subscribe("sending");
      mqttClient.subscribe("servoangle");
    }
    else{
      Serial.print("Failed");
...
void receiveCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Convert byte payload to null-terminated char array
...
void lightintensity(){
  ...
  maxReading= sendingInterval/samplingInterval;
  ...
  if (currentMillis - previousMillis >= (samplingInterval*1000)) {
    ...
    if( readingIndex==maxReading){
      ...
      Serial.println(sum);
      delay(1000);
      float average = sum / float(maxReading);

      Serial.print("average: ");
      Serial.println(average);
      mqttClient.publish("esp32/light", String(average).c_str());
...
if(!mqttClient.connected()){
  display.clearDisplay();
  print_line("Reconnecting To MQTT", 2, 0, 0);
  connectToBroker();
}
  mqttClient.loop();

 
 mqttClient.publish("esp32/temp", String(temp).c_str());
 delay(500);

 ldrValue = analogRead(LDR);

 //Serial.println(ldrValue);
 //mqttClient.publish("esp32/light", String(ldrValue).c_str());
